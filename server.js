...
try{
	...
	// pdfkit optional - if not installed, PDF export will return 501
}
...
app.get("/api/export",auth,async(req,res)=>{
	try{
		...
		// If accountant requests export for specific user, allow only if current user is contabilista
		let targetUserId = req.session.userId
		if(req.query.user_id){
			try{
				const me = await pool.query("SELECT tipo_entidade FROM users WHERE id=$1",[req.session.userId])
				const tipo = me.rows[0] && me.rows[0].tipo_entidade ? String(me.rows[0].tipo_entidade).toLowerCase() : ""
				if(tipo === 'contabilista' || tipo === 'contabilista'){
					const uid = Number(req.query.user_id)
					if(Number.isInteger(uid) && uid>0) targetUserId = uid
				}
			}catch(_){ }
		}
		...
		if(req.query.from){
			const fromNorm = dataParaInput(req.query.from)
			if(fromNorm) {
				params.push(fromNorm)
				whereClauses.push(`data >= $${params.length}`)
			}
		}
		if(req.query.to){
			const toNorm = dataParaInput(req.query.to)
			if(toNorm){
				params.push(toNorm)
				whereClauses.push(`data <= $${params.length}`)
			}
		...
		const sql = `SELECT id, data, fornecedor, tipo, COALESCE(valor_total,valor,0) AS total, COALESCE(valor_iva,0) AS iva, COALESCE(valor_sem_iva,0) AS semiva, nif, ficheiro FROM registos WHERE ${whereClauses.join(' AND ')} ORDER BY data DESC`
		const result = await pool.query(sql, params)

		const rows = result.rows || []

		const nowS = new Date().toISOString().slice(0,10)

		if(format === 'csv' || format === 'excel'){
			// Columns matching dashboard: Data, Fornecedor/Cliente, Total, IVA, Sem IVA, Tipo
			const cols = ['data','fornecedor','tipo','total','iva','semiva','nif','ficheiro']
			const lines = []
			lines.push(cols.join(','))
			for(const r of rows){
				const v = []
				v.push(String(r.data || ''))
				v.push('"' + String((r.fornecedor||'').replace(/"/g,'""')) + '"')
				v.push(String(r.tipo || ''))
				v.push(String(Number(r.total || 0).toFixed(2)))
				v.push(String(Number(r.iva || 0).toFixed(2)))
				v.push(String(Number(r.semiva || 0).toFixed(2)))
				v.push(String(r.nif || ''))
				v.push(String(r.ficheiro || ''))
				lines.push(v.join(','))
			}
			const csv = lines.join('\n')
			res.setHeader('Content-Type','text/csv; charset=utf-8')
			res.setHeader('Content-Disposition', `attachment; filename="registos-${nowS}.csv"`)
			return res.send(csv)
		}

		if(format === 'pdf'){
			if(!PDFDocument){
				return res.status(501).send('PDF export not available on this server')
			}

			res.setHeader('Content-Type','application/pdf')
			res.setHeader('Content-Disposition', `attachment; filename="registos-${nowS}.pdf"`)

			const doc = new PDFDocument({margin:40, size:'A4'})
			doc.pipe(res)

			doc.fontSize(18).text('Relatório de Registos', {align:'center'})
			doc.moveDown(0.5)
			doc.fontSize(10).text(`Exportado em: ${new Date().toLocaleString()}`)
			doc.moveDown(0.8)

			const tableTop = doc.y
			const itemHeight = 18

			// Header (Data, Fornecedor, Tipo, Total, IVA, Sem IVA)
			doc.fontSize(10).text('Data',40,tableTop)
			doc.text('Fornecedor',100,tableTop)
			doc.text('Tipo',320,tableTop)
			doc.text('Total',380,tableTop,{width:70,align:'right'})
			doc.text('IVA',460,tableTop,{width:70,align:'right'})
			doc.text('Sem IVA',540,tableTop,{width:70,align:'right'})
			doc.moveDown(0.5)

			let y = tableTop + 18
			for(const r of rows){
				if(y > 720){ doc.addPage(); y = 40 }
				const dateStr = r.data ? String(r.data) : ''
				doc.fontSize(10).text(dateStr,40,y)
				doc.text(String(r.fornecedor || ''),100,y,{width:200})
				doc.text(String(r.tipo || ''),320,y)
				doc.text(Number(r.total || 0).toFixed(2) + ' EUR',380,y,{width:70,align:'right'})
				doc.text(Number(r.iva || 0).toFixed(2) + ' EUR',460,y,{width:70,align:'right'})
				doc.text(Number(r.semiva || 0).toFixed(2) + ' EUR',540,y,{width:70,align:'right'})
				y += itemHeight
			}
		...
		res.status(500).send('Erro ao exportar registos')
	}
})